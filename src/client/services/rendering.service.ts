...
class RenderingService {
  ...
  public async initialize(
    ...
    if (this.app) {
      console.log(
        '[RenderingService] App already exists, returning existing initialization promise'
      );
    ...
    if (this.isInitialized) {
      console.warn(
        '[RenderingService] isInitialized exists but app is null, cleaning up and reinitializing'
      );
    ...
    try {
      ...
      console.log(
        `[RenderingService] Creating viewport with world dimensions: ${worldWidth}x${worldHeight}`
      );
      ...
      // --- Activate Plugins ---
      const initialZoomLimits = this.calculateZoomLimits(
      ...
      if (savedState) {
        ...
        if (spawnPosition) {
          ...
          console.log(
            `[RenderingService] Centered map on player spawn at (${spawnPosition.x}, ${spawnPosition.y})`
          );
        } else {
        ...
        for (const entry of entries) {
          ...
          this.resizeTimeout = setTimeout(() => {
            ...
            if (this.viewport && this.mapRenderer) {
              ...
              this.mapRenderer.render(this.viewport);
              console.log(
                `[RenderingService] Viewport resized and re-rendered to ${width}x${height}.`
              );
            }
          }, 100); // 100ms debounce
        }
  ...
  public async updateMap(newMapData: MapData): Promise<void> {
    console.log(
      `[RenderingService] updateMap called with ${newMapData.width}x${newMapData.height} map, ${newMapData.tiles.length} tiles`
    );
   ...
   */
  private findPlayerSpawnPosition(
    gameState: ClientGameStatePayload,
    mapData: MapData,
    currentUserId: string
    ...
    if (!spawnTile) {
      console.warn(
        `[RenderingService] No spawn ${targetSpawnNumber} found for player index ${playerIndex}`
      );
      // Fallback to first available spawn
      const fallbackSpawn = sortedSpawns[0];
      if (fallbackSpawn) {
        const fallbackPosition = this.hexToPixel(
          fallbackSpawn.coordinates.q,
          fallbackSpawn.coordinates.r
        );
        console.log(
          `[RenderingService] Using fallback spawn ${fallbackSpawn.playerSpawn} for player ${currentUserId}`
        );
    ...
    console.log(
      `[RenderingService] Player ${currentUserId} (index ${playerIndex}) assigned to P${targetSpawnNumber} spawn at hex (${q}, ${r}) -> pixel (${pixelPosition.x}, ${pixelPosition.y})`
    );
