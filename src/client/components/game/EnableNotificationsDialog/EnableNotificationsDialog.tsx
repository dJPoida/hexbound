...
export const EnableNotificationsDialog = ({
  onComplete,
  permissionState,
  ...
  const [currentPermission, setCurrentPermission] =
    useState<NotificationPermission>(permissionState);
    ...
    try {
      ...
      if (subscription) {
        ...
        if (currentPermission === 'denied') {
          ...
          setError(
            'Permission was not granted. You can enable notifications later in Game Settings.'
          );
        }
      }
    } catch (err) {
  ...
  return (
    <Dialog title='Get Turn Alerts!' footer={footer}>
      <Text variant='body'>
        Hexbound is best with notifications enabled. We&apos;ll send you a quick alert when
      ...
      {currentPermission === 'denied' && (
        <div class={styles.warningContainer}>
          <Text as='div' class={styles.warningText}>
            ...
            Your browser is currently blocking notifications. To enable them, please click the lock
            icon in your address bar and change the permission.
          </Text>
        </div>
      )}

      <Text variant='caption' color='subtle'>
        This helps keep the game moving and ensures you don&apos;t miss out on the action! You can
