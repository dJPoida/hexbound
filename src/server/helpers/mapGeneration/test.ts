...
export function testMapGeneration(): void {
  ...
  try {
    ...
    for (const tile of result.mapData.tiles) {
      ...
      if (
        tile.coordinates.q < 0 ||
        tile.coordinates.q >= 50 ||
        tile.coordinates.r < 0 ||
        tile.coordinates.r >= 30
      ) {
    ...
    console.log(
      `[Summary] Generated ${result.totalTilesGenerated} tiles across ${result.passResults.length} passes`
    );
