...
export class RefactorGameStatus1749435312595 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE "game_statuses" ("statusId" SERIAL NOT NULL, "statusName" character varying NOT NULL, CONSTRAINT "UQ_f12c1e8a600632a79d85752c018" UNIQUE ("statusName"), CONSTRAINT "PK_4683075a034262453e9944357c9" PRIMARY KEY ("statusId"))`
    );

    // 2. Seed the new table with our status values
    await queryRunner.query(
      `INSERT INTO "game_statuses" ("statusName") VALUES ('${GameStatusValues.WAITING}'), ('${GameStatusValues.ACTIVE}'), ('${GameStatusValues.FINISHED}')`
    );
    ...
    const waitingStatus = await queryRunner.query(
      `SELECT "statusId" FROM "game_statuses" WHERE "statusName" = '${GameStatusValues.WAITING}'`
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "games" ADD "status" character varying NOT NULL DEFAULT 'waiting'`
    );
